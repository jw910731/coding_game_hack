...
class IRobot {
public:
    virtual void Init(const uint32_t x, const uint32_t y, uint64_t* const step, Maze* const mz);
    virtual void Walk();
};
#pragma endregion
...
public:
    ~CDGRobot();
    void                                 Init(const uint32_t x, const uint32_t y, uint64_t* const step, Maze* const mz) override;
    void                                 Walk() override;
