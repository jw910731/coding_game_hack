...
void           IRobot::Init(const uint32_t x, const uint32_t y, uint64_t* const step, Maze* const mz)
{
    throw new NotImplementedError();
}
void IRobot::Walk() { throw new NotImplementedError(); }
#pragma endregion
...
void CDGRobot::Init(const uint32_t x, const uint32_t y, uint64_t* const step, Maze* const mz)
{
...
const std::pair<uint32_t, uint32_t>& CDGRobot::GetPos() { return *this->_pos; }
const int                            CDGRobot::GetDir() { return this->_dir; }
#pragma endregion
#pragma dp_robot
void CDGDPRobot::Init(const uint32_t x, const uint32_t y, uint64_t* const step, Maze* const mz)
{
...
#ifdef DEBUG
        std::cerr << "[step]" << this->_dpmap[curpos.second][curpos.first][curdir] << '\n';
#endif

        *this->_step %= this->_dpmap[curpos.second][curpos.first][curdir];
    }
    else
        this->_dpmap[curpos.second][curpos.first][curdir] = 0;
}
const std::pair<uint32_t, uint32_t>& CDGDPRobot::GetPos() { return this->_robot->GetPos(); }
